    ...
    for result_file in reversed(sorted(L_result_files, key=lambda e:e['worker_id'])):
        ...
        for (k, c2) in D_colors.items():
            if D_logged.has_key(k):
                if worker_id == 0:
                    color = c2[0]
                    h = pylab.plot(L_step, D_logged[k], c=color, label=k)
                    #L_legend_handles.append(h)
                    #L_legend_tags.append(k)
                else:
                    color = c2[1]
                    h = pylab.plot(L_step, D_logged[k], c=color)
    
    #plt.legend(L_legend_handles, L_legend_tags)
    plt.legend()
...
def find_result_files(dir):
    ...
    for path in L_files:
        ...
        if m:
            L_result_files.append({'path' : path, 'worker_id' : int(m.group(1))})
